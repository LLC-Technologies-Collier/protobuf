...
#define DECLARE_CORO_STATE() \
    ...
    void coro_finish(int id) { coro_active[id - 1] = 0; coro_transfer(coro_ctxs[id - 1], &main_ctx); }
...
#define RUN_CORO_TEST(coro_test_func, args) \
    do { \
        ...
        for (int i = 0; i < NUM_COROS; i++) { \
            ...
            char test_name[100]; \
            snprintf(test_name, sizeof(test_name), "Coro %d completed without errors", i + 1); \
