...
subtest 'Pure-Perl Serialize/Parse' => sub {
    ...
    my $msg = Test::Test::TestMessage->new({ profile => 'pure_perl' });
    
    ok($msg->isa('Test::Test::TestMessage'), 'Object is correct class');
    ...
    my $decoded = Test::Test::TestMessage->parse($bin, { profile => 'pure_perl' });
...
subtest 'Pure-Perl Nested Messages' => sub {
    ...
    my $nested = Test::Test::NestedMessage->new({ profile => 'pure_perl' });
...
subtest 'Pure-Perl vs XS Interop' => sub {
    # Create XS message
    my $xs_msg = Test::Test::TestMessage->new({ value => 789, test_string => 'interop' });
    my $bin = $xs_msg->serialize();
    
    # Parse with PurePerl
    # Note: parse() is a class method. We need to pass profile in options.
    # Currently parse() takes ($data, $options).
    my $pp_msg = Test::Test::TestMessage->parse($bin, { profile => 'pure_perl' });
