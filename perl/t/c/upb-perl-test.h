...
#define isnt(got, expected, name) fprintf(stderr, "%*s%s %d - %s\n", indent_level * 4, "", ((got) != (expected)) ? "ok" : "not ok", ++test_num, (name)); if ((got) == (expected)) { fprintf(stderr, "%*s  # Got: %ld (Expected different)\n", indent_level * 4, "", (long)(got)); }
...
#define isnt_u(got, expected, name) fprintf(stderr, "%*s%s %d - %s\n", indent_level * 4, "", ((got) != (expected)) ? "ok" : "not ok", ++test_num, (name)); if ((got) == (expected)) { fprintf(stderr, "%*s  # Got: %" PRIu64 " (Expected different)\n", indent_level * 4, "", (uint64_t)(got)); }
