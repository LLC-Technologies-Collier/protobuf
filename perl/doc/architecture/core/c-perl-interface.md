...
5.  **Thread-Safe Global Freezing**: (Planned) Implement a mechanism to "freeze" a `DescriptorPool`, allowing it to be safely shared across multiple Perl interpreters (ithreads) without deep cloning, leveraging shared C-level pointers.
6.  **TSan-Verified Concurrency**: (Planned) The interface and underlying C-state management are verified using ThreadSanitizer (TSan) during continuous integration to guarantee race-free operation in multi-threaded environments.
