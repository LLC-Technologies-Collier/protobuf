...
void PerlCodeGenerator::BuildPackageMap() {
  ...
  for (size_t i = 0; i < file_count; i++) {
    ...
    std::string_view name(name_sv.data, name_sv.size);
    file_to_package_[std::string(name)] = GetPerlPackage(files[i]);
  }
}

void PerlCodeGenerator::RegisterTypesRecursively(
    const proto2_DescriptorProto* msg_proto, std::string_view proto_prefix,
...
std::string PerlCodeGenerator::ResolvePerlClass(std::string_view proto_type) {
  auto it = type_to_perl_class_.find(std::string(proto_type));
  if (it != type_to_perl_class_.end()) {
    return it->second;
  }
  std::string_view s = proto_type;
  ConsumePrefix(&s, ".");
...
std::string PerlCodeGenerator::GetPerlPackage(
  ...
  std::string_view pkg(pkg_sv.data, pkg_sv.size);

  std::string_view file_segment = name;
...
proto2_compiler_CodeGeneratorResponse* PerlCodeGenerator::Generate() {
  ...
  for (size_t i = 0; i < gen_file_count; i++) {
    std::string_view name(gen_files[i].data, gen_files[i].size);
    ...
    for (size_t j = 0; j < all_file_count; j++) {
      ...
      if (name == std::string_view(cur_name_sv.data, cur_name_sv.size)) {
...
std::string PerlCodeGenerator::GenerateModule(
  ...
  for (size_t i = 0; i < dep_count; i++) {
    std::string_view dep(deps[i].data, deps[i].size);
  ...
  for (size_t i = 0; i < message_count; i++) {
    ...
    std::string_view msg_name(msg_name_sv.data, msg_name_sv.size);
    ...
    for (size_t j = 0; j < field_count; j++) {
      ...
      ss << "    # Field: "
         << std::string_view(f_name_sv.data, f_name_sv.size);
      ss << " Type: " << f_type << " ("
         << std::string_view(f_type_name_sv.data, f_type_name_sv.size)
         << ")\n";
    }
    ss << "\n";
    ...
    for (size_t j = 0; j < enum_count; j++) {
      ...
      std::string_view e_name(e_name_sv.data, e_name_sv.size);
      ...
      for (size_t k = 0; k < val_count; k++) {
        ...
        ss << "const my $" << msg_name << "_"
           << std::string_view(ev_name_sv.data, ev_name_sv.size) << " => "
           << ev_num << ";\n";
      }
      ss << "\n";
    }
  ...
  upb_StringView pkg_sv = proto2_FileDescriptorProto_package(file_proto);
  std::string_view proto_pkg(pkg_sv.data, pkg_sv.size);
  ...
  for (size_t i = 0; i < service_count; i++) {
    ...
    std::string_view srv_name(srv_name_sv.data, srv_name_sv.size);
...
void PerlCodeGenerator::PrintTypesRecursively(
    const proto2_DescriptorProto* msg_proto, std::string_view current_package,
    std::stringstream& ss) {
  upb_StringView msg_name_sv = proto2_DescriptorProto_name(msg_proto);
  std::string_view msg_name(msg_name_sv.data, msg_name_sv.size);
  std::string full_msg_name = std::string(current_package) + "::" + std::string(msg_name);
...
void PerlCodeGenerator::PrintMessageTypes(
  ...
  upb_StringView msg_name_sv = proto2_DescriptorProto_name(msg_proto);
  std::string_view msg_name(msg_name_sv.data, msg_name_sv.size);
  std::string full_perl_class = std::string(current_package) + "::" + std::string(msg_name);
...
void PerlCodeGenerator::PrintEnumTypes(
  ...
  std::string_view enum_name(enum_name_sv.data, enum_name_sv.size);
