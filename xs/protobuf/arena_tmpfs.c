...
static inline int ftruncate(int fd, size_t size) {
    return 0; // Stub
}
static inline int win_close(int fd) {
    return _close(fd);
}
static inline int win_open(const char* path, int flags, ...) {
    return -1; // Tmpfs file IPC is disabled/not supported natively on Windows
}
...
static inline int mprotect(void* addr, size_t length, int prot) {
    ...
    if (prot & (PROT_READ | PROT_WRITE)) protect = PAGE_READWRITE;
